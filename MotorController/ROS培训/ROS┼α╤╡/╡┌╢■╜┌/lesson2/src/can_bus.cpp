...
namespace sp_hw
{
    ...
    void CanBus::write()
    {
        ...
        {
            double cmd = limitAmplitude(0.015 * eff2act, 10000.0);
            if (1 <= id && id <= 4)
            {
    ...
    void CanBus::read(ros::Time time)
    {
        ...
        for (const CanFrameStamp &can_frame_stamp : read_buffer_)
        {
            ...
            ROS_INFO_STREAM("position: " << position << "rad");
            ROS_INFO_STREAM("velocity: " << velocity << "rad/s");
            ROS_INFO_STREAM("effort: " << effort << "N.m");
