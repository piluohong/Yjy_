...
class CANMotorController:
    PARAM_TABLE = {
        "motorOverTemp": {"feature_code": 0x200D, "type": "int16"}, #电机保护温度
        "overTempTime": {"feature_code": 0x200E, "type": "int32"}, #过温时间
        "limit_torque": {"feature_code": 0x2007, "type": "float"}, #转矩限制
        "cur_kp": {"feature_code": 0x2012, "type": "float"}, #电流kp
        "cur_ki": {"feature_code": 0x2013, "type": "float"}, #电流ki
        "spd_kp": {"feature_code": 0x2014, "type": "float"}, #速度kp
        "spd_ki": {"feature_code": 0x2015, "type": "float"}, #速度ki
        "loc_kp": {"feature_code": 0x2016, "type": "float"}, #位置kp
        "spd_filt_gain": {"feature_code": 0x2017, "type": "float"}, #速度滤波参数
        "limit_spd": {"feature_code": 0x2018, "type": "float"}, #位置模式速度限制
        "limit_cur": {"feature_code": 0x2019, "type": "float"}, #位置，速度模式电流限制
    }

    PARAMETERS = {
        "run_mode": {"index": 0x7005, "format": "u8"}, #电机运行模式
        "iq_ref": {"index": 0x7006, "format": "f"}, #电流模式Iq指令 -23A-23A
        "spd_ref": {"index": 0x700A, "format": "f"}, #转速模式转速指令
        "limit_torque": {"index": 0x700B, "format": "f"}, #转矩限制
        "cur_kp": {"index": 0x7010, "format": "f"}, #电流kp 默认0.125
        "cur_ki": {"index": 0x7011, "format": "f"}, #电流ki
        "cur_filt_gain": {"index": 0x7014, "format": "f"}, #电流滤波系数 0.1
        "loc_ref": {"index": 0x7016, "format": "f"}, #位置模式角度指令
        "limit_spd": {"index": 0x7017, "format": "f"}, #位置模式速度限制
        "limit_cur": {"index": 0x7018, "format": "f"}, #速度，位置模式电流限制
    }
    TWO_BYTES_BITS = 16
