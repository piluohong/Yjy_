...
void DriveWidget::sendVelocitiesFromMouse( int x, int y, int width, int height )
{
  ...
  linear_velocity_ = (1.0 - float( y - vpad ) / float( size / 2 )) * linear_scale_; //调正虚拟遥感的最大速度映射值
  if ( linear_velocity_ < -2.0 ) linear_velocity_ = -2.0;
  else if ( linear_velocity_ > 2.0 ) linear_velocity_ = 2.0;
  if ( fabs( linear_velocity_ ) < 0.05 ) linear_velocity_ = 0;
