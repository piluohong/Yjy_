...
def listener():
  ...
  rospy.init_node('gui')
  rospy.Subscriber("/time_duration_gui", Float32, timeDurationCallback)
  rospy.Subscriber("/linear_x", Float32, runTimeCallback)
  rospy.Subscriber("/angular_z", Float32, exploredVolumeCallback)
  rospy.Subscriber("/traveling_distance_gui", Float32, travelingDistanceCallback)

  fig=plt.figure(figsize=(8,7))
  fig1=fig.add_subplot(311)
  plt.title("Exploration Metrics\n", fontsize=14)
  plt.margins(x=0.001)
  fig1.set_ylabel("Angular\nZ (rad/s)", fontsize=12)
  l1, = fig1.plot(time_list2, explored_volume_list, color='r', label='Angular Z')
  ...
  fig3.set_ylabel("Linear\nX (m/s)", fontsize=12)
  fig3.set_xlabel("Time Duration (s)", fontsize=12) #only set once
  l3, = fig3.plot(time_list1, run_time_list, color='r', label='Linear X')
  ...
  while not rospy.is_shutdown():
      ...
      if count >= 100:
        ...
        fig1.set_ylim(-2, max_explored_volume + 2) #angular
        fig1.set_xlim(start_time_duration, time_duration + 10)
        fig2.set_ylim(0, max_traveling_diatance + 10) 
        fig2.set_xlim(start_time_duration, time_duration + 10)
        fig3.set_ylim(-2, max_run_time + 2) #linear
